  ...
  -->
	...
	<xsl:template match="/">
		...
		<xsl:if test="count(//GetOrderDetailsInfoResults/LineItem[pkg_no=0]) &gt; 0">
			...
			<!-- ARRIVAL DATE and GUESTS -->
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td bgcolor="#1a3869" style="text-align:center; font-family:'Open Sans', Tahoma, Arial,sans-serif,; font-size:30px; line-height:35px; text-align:center; padding-top:15px; padding-bottom:15px;" width="48%">
						<span id="arrival-date" style="color:#fff;">
					...
					<td bgcolor="#1a3869" style="text-align:center; font-family:'Open Sans', Tahoma, Arial,sans-serif,; font-size:30px; line-height:35px; text-align:center; padding-top:15px; padding-bottom:15px;" width="48%">
						<span id="guests" style="color:#fff;">
							<xsl:value-of select=" count(//GetOrderDetailsInfoResults/SubLineItem[li_seq_no = //GetOrderDetailsInfoResults/LineItem[perf_desc = 'Mansion Tours']/li_seq_no]/sli_no)"/> guests
						</span>
					</td>
			...
			<table style="width:100%;">
				<tbody>
					<!--display line items that ARE Mansion Tours -->
					<xsl:for-each select="//GetOrderDetailsInfoResults/LineItem[pkg_no=0 and perf_desc = 'Mansion Tour']">
						...
						<tr>
							<!-- ticket tour number of seats -->
							<td width="9%" height="60" bgcolor="#ffffff" style="border: 1px solid #e3e0db; color:#64605a; font-family: 'Open Sans', Tahoma, Arial, sans-serif; font-size:15px; line-height:20px; text-align:center; font-weight: bold;">
							...
							<!-- ticket tour name -->
							<td width="90%" height="60" bgcolor="#ffffff" style="border: 1px solid #e3e0db; color:#64605a; font-family: 'Open Sans', Tahoma, Arial, sans-serif; font-size:15px; line-height:20px; text-transform: uppercase;">
								<table>
									<tr>
										<td width="12"></td>
										<td style="color:#64605a; font-family: 'Open Sans', Tahoma, Arial, sans-serif; font-size:15px; line-height:20px; text-transform: uppercase;">
											<xsl:value-of select="perf_desc"/> (<xsl:value-of select="//GetOrderDetailsInfoResults/SubLineItem[li_seq_no=$liNo]/price_type_desc"/>)
										</td>
									</tr>
											...
											<table>
												<tr>
													<td style="padding:0px; line-height: 0px; font-size:0px;">
														<img src="http://s3.amazonaws.com/mtv-main-assets/files/resources/tessitura-email_time_ico.jpg" border="0" width="12" height="12" alt="" />
													</td>
													<td style="color:#64605a; font-family: 'Open Sans', Tahoma, Arial, sans-serif; font-size:15px; line-height:20px;">
														<xsl:value-of select="//GetOrderDetailsInfoResults/SubLineItem[li_seq_no=$liNo]/zone_desc"/>
													</td>
												</tr>
											</table>
					...
					<!--display line items that are NOT Mansion Tours -->
					<xsl:for-each select="//GetOrderDetailsInfoResults/LineItem[pkg_no=0 and perf_desc != 'Mansion Tour']">
						<xsl:variable name="liNo" select="li_seq_no"/>
						<xsl:variable name="total" select="sum(//GetOrderDetailsInfoResults/SubLineItem[li_seq_no=$liNo]/due_amt)"/>
						<xsl:variable name="perfDesc" select="//GetOrderDetailsInfoResults/LineItem[pkg_no=0]/perf_desc"/>
						<tr>
							<!-- ticket tour number of seats -->
							<td width="8%" height="60" bgcolor="#ffffff" style="border: 1px solid #e3e0db; color:#64605a; font-family: 'Open Sans', Tahoma, Arial, sans-serif; font-size:15px; line-height:20px; text-align:center; font-weight: bold;">
								<xsl:value-of select="count(//GetOrderDetailsInfoResults/SubLineItem[li_seq_no=$liNo])"/>
							</td>
					...
					<tr>
						...
						<!-- ticket tour name -->
						<td width="75%" style="color:#64605a; font-family: 'Open Sans', Tahoma, Arial, sans-serif; font-size:22px; line-height:24px; text-align:center; font-weight: bold;">
							<span id="total">
								ORDER SUBTOTAL: <xsl:value-of select="format-number($thisTotal, '$####.00', 'dollar')" />
								<br>
								</br>
								FEES: <xsl:value-of select="format-number($fee_total, '$####.00', 'dollar')" />
							</span>
						</td>
